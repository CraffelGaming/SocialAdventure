...
router.put('/' + endpoint + '/:node/', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (channel) {
        if (global.isMaster(request, response, node)) {
            response.status(yield locationItem_1.LocationItem.put({ sequelize: channel.database.sequelize, element: request.body })).json(request.body);
        }
        else {
    ...
    if (channel) {
        if (global.isMaster(request, response, node)) {
            if (request.params.handle != null) {
                const item = yield channel.database.sequelize.models.location.findByPk(request.params.handle);
                if (item) {
                    yield channel.database.sequelize.models.location.destroy({ where: { handle: request.params.handle } });
                }
...
//# sourceMappingURL=api.location.js.map