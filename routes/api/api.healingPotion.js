...
router.get('/' + endpoint + '/:node/', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (channel) {
        const item = yield channel.database.sequelize.models.healingPotion.findAll({ order: [['handle', 'ASC']], raw: false });
        if (item)
    ...
    if (channel) {
        if (global.isMaster(request, response, node)) {
            if (request.params.handle != null) {
                const item = yield channel.database.sequelize.models.healingPotion.findByPk(request.params.handle);
                if (item) {
                    yield channel.database.sequelize.models.healingPotion.destroy({ where: { handle: request.params.handle } });
                }
