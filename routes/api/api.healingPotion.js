...
router.get('/' + endpoint + '/:node/', async (request, response) => {
    try {
        ...
        if (channel) {
            const item = await channel.database.sequelize.models.healingPotion.findAll({ order: [['handle', 'ASC']] });
            if (item)
        ...
        if (channel) {
            if (global.isMaster(request, response, node)) {
                if (request.params.handle != null) {
                    const item = await channel.database.sequelize.models.healingPotion.findByPk(request.params.handle);
                    if (item) {
...
router.post('/' + endpoint + '/:node/heal/:handle/hero/:name', async (request, response) => {
    try {
