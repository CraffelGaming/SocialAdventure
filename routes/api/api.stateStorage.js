...
router.get('/' + endpoint + '/:name', async (request, response) => {
    try {
...
router.put('/' + endpoint + '/', async (request, response) => {
    try {
        if (global.isRegistered(request, response)) {
...
router.delete('/' + endpoint + '/:name', async (request, response) => {
    try {
        global.worker.log.trace(`delete ${endpoint}, name ${request.params.name}`);
        if (global.isRegistered(request, response)) {
            const item = await global.worker.globalDatabase.sequelize.models.stateStorage.findOne({ where: { handle: request.params.name, channelName: request.session.userData.login } });
            if (item) {
