...
router.get('/' + endpoint + '/:node/', async (request: express.Request, response: express.Response) => {
    ...
    if(channel) {
        const item = await channel.database.sequelize.models.heroTrait.findAll({order: [ [ 'heroName', 'ASC' ]], raw: false, include: [{
...
router.get('/' + endpoint + '/:node/hero/:name', async (request: express.Request, response: express.Response) => {
    ...
    if(channel) {
        const item = await channel.database.sequelize.models.heroTrait.findAll({where: { heroName: request.params.name }, raw: false});
        if(item) response.status(200).json(item);
        else response.status(404).json();
    } else response.status(404).json();
});
