        ...
        if(channel) {
            if(request.query.childs !== "false"){
                item = await channel.database.sequelize.models.historyDuell.findAll({order: [ [ 'handle', 'ASC' ]], include: [{
                    ...
                    model: channel.database.sequelize.models.hero,
                    as: 'heroTarget',
                }]}) as Model<HistoryDuellItem>[];
            } else item = await channel.database.sequelize.models.historyDuell.findAll({order: [ [ 'handle', 'ASC' ]]}) as Model<HistoryDuellItem>[];
...
router.get('/' + endpoint + '/:node/sourcehero/:name', async (request: express.Request, response: express.Response) => {
    try{
        ...
        if(channel) {
            ...
            } else item = await channel.database.sequelize.models.historyDuell.findOne({where: { sourceHeroName: request.params.name }}) as Model<HistoryDuellItem>;
...
router.get('/' + endpoint + '/:node/targethero/:name', async (request: express.Request, response: express.Response) => {
    try{
        ...
        if(channel) {
            ...
            } else item = await channel.database.sequelize.models.historyDuell.findOne({where: { targetHeroName: request.params.name }}) as Model<HistoryDuellItem>;
