...
router.get('/' + endpoint + '/:node/hero/:name', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (channel) {
        if (request.query.childs !== "false") {
            item = (yield channel.database.sequelize.models.heroWallet.findOne({ where: { heroName: request.params.name }, raw: false, include: [{
                        model: channel.database.sequelize.models.hero,
                        as: 'hero',
                    }] }));
        }
        else
            item = (yield channel.database.sequelize.models.heroWallet.findOne({ where: { heroName: request.params.name }, raw: false }));
        if (item)
            response.status(200).json(item);
        else
            response.status(404).json();
