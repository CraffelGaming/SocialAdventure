...
router.get('/' + endpoint + '/:node/', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (channel) {
        const item = yield channel.database.sequelize.models.heroWallet.findAll({ order: [['heroName', 'ASC']], raw: false, include: [{
...
router.get('/' + endpoint + '/:node/hero/:name', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const channel = global.worker.channels.find(x => x.node.name === node);
    if (channel) {
        const item = yield channel.database.sequelize.models.heroWallet.findAll({ where: { heroName: request.params.name }, raw: false });
        if (item)
            response.status(200).json(item);
        else
            response.status(404).json();
