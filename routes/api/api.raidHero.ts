...
router.get('/' + endpoint + '/:node/:raidHandle/:heroName', async (request: express.Request, response: express.Response) => {
    try{
        ...
        if(channel) {
            const item = await channel.database.sequelize.models.enemy.findAll({ where: { raidHandle: request.params.raidHandle, heroName: request.params.heroName }, order: [ [ 'name', 'ASC' ]], raw: false });
            if(item) response.status(200).json(item);
            else response.status(404).json();
        } else response.status(404).json();
    } catch(ex){
        ...
        if(channel) {
            if(global.isMaster(request, response, node)){
                if(request.params.handle != null){
                    const item = await channel.database.sequelize.models.enemy.findByPk(request.params.handle) as unknown as RaidHeroItem;
                    if(item){
