...
router.put('/' + endpoint + '/:node/', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (channel) {
        if (global.isMaster(request, response, node)) {
            response.status(yield promotionItem_1.PromotionItem.put({ sequelize: channel.database.sequelize, element: request.body })).json(request.body);
        }
        else {
...
router.post('/' + endpoint + '/:node/redeem/:promotionHandle/:heroName', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const channel = global.worker.channels.find(x => x.node.name === node.name);
    if (channel) {
        if (global.isMaster(request, response, node) || global.isHero(request, response, request.params.heroName) && request.params.promotionHandle !== "NewStart") {
