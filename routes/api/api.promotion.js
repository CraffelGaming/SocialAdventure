...
router.get('/' + endpoint + '/:node/', async (request, response) => {
    try {
        ...
        if (channel) {
            const item = await channel.database.sequelize.models.promotion.findAll({ order: [['handle', 'ASC']] });
            if (item)
        ...
        if (channel) {
            if (global.isMaster(request, response, node)) {
                if (request.params.handle != null) {
                    const item = await channel.database.sequelize.models.promotion.findByPk(request.params.handle);
                    if (item) {
...
router.post('/' + endpoint + '/:node/redeem/:promotionHandle/:heroName', async (request, response) => {
    try {
        ...
        const channel = global.worker.channels.find(x => x.node.getDataValue('name') === node.name);
        if (channel) {
            const promotion = await channel.database.sequelize.models.promotion.findByPk(request.params.promotionHandle);
            if (promotion) {
