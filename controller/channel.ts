...
export class Channel {
    ...
    async addSay(){
        const translation = await global.worker.globalDatabase.sequelize.models.translation.findAll({where: { page: 'say', language: this.node.language }, order: [ [ 'handle', 'ASC' ]], raw: false}) as unknown as TranslationItem[]
        ...
        for(const item of Object.values(await this.database.sequelize.models.say.findAll({order: [ [ 'command', 'ASC' ]], raw: true})) as unknown as SayItem[]){
            this.say.push(new Say(translation, this, item));
        }
        global.worker.log.trace(this.say);
    }

    async addLoot(){
        const translation = await global.worker.globalDatabase.sequelize.models.translation.findAll({where: { page: 'loot', language: this.node.language }, order: [ [ 'handle', 'ASC' ]], raw: false}) as unknown as TranslationItem[]
